...
(function(){
  ...
  function renderBatch() {
    if(window.__hwLogPush) window.__hwLogPush('renderBatch() INICIO, LOADED=' + LOADED + ' activeFilter=' + activeFilter);
    ...
    if(window.__hwLogPush) window.__hwLogPush('renderBatch() batch.length=' + batch.length + ' (posts.length total=' + posts.length + ')');
    ...
    LOADED += batch.length;
    if(window.__hwLogPush) window.__hwLogPush('renderBatch() FIN, LOADED ahora=' + LOADED + ' container.children=' + container.children.length);
  ...
  function initObserver() {
    ...
    var observer = new IntersectionObserver(function(entries) {
      if(window.__hwLogPush) window.__hwLogPush('Jekyll observer: isIntersecting=' + entries[0].isIntersecting + ' LOADING=' + LOADING + ' _cfLoading=' + _cfLoading + ' LOADED=' + LOADED + ' total=' + getFilteredPosts().length);
  ...
  function initFeed() {
    fetch('/assets/data/posts.json')
      ...
      .then(function(data) {
        ...
        /* Pintar la primera tanda YA, sin depender del observer.
           El feed-sentinel puede quedar fuera del viewport inicial si
           el community feed (D1) ocupa espacio arriba — el observer
           solo debe encargarse de cargar MÁS posts al hacer scroll,
           nunca de la carga inicial. */
        if (LOADED === 0 && activeFilter !== 'following' && !LOADING) {
          LOADING = true;
          renderBatch();
        }
        initObserver();
