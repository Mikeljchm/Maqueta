...
async function handleProfile(request, env, corsH) {
  ...
  if (request.method === 'GET') {
    ...
    const { results } = await env.DB.prepare(
      'SELECT username, display_name, bio, avatar_url, banner_url, age, age_public, city, country FROM user_profiles WHERE user_id=?'
    ).bind(targetId).all();
    return apiJson({ username: results[0]?.username||null, display_name: results[0]?.display_name||'', bio: results[0]?.bio||'', avatar_url: results[0]?.avatar_url||'', banner_url: results[0]?.banner_url||'', age: results[0]?.age||null, age_public: results[0]?.age_public||0, city: results[0]?.city||'', country: results[0]?.country||'' }, 200, corsH);
  }
