...
export default {
  async fetch(request, env, ctx) {
    ...
    if (path.startsWith('/api/')) {
      ...
      /* TEMPORAL: para ver exactamente que email/nombre devuelve Google,
         ya que adivinar el correo a mano fallo dos veces. Sacar despues
         de confirmar el valor real. */
      if (path === '/api/debug-whoami') {
        return apiJson({
          session: getSession(request),
          admin_login_recibido: request._verifiedAdminLogin
        }, 200, corsH);
      }
